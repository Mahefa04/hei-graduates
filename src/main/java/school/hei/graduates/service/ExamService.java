...
public class ExamService {
    ...
    public List<ExamResponse> getByCourseOfferingId(UUID courseOfferingId) {
        ...
        return examRepository.findByCourseOffering_Id(courseOfferingId)
                .stream()
                .map(examMapper::toResponse)
                .toList();
    }
    ...
    public ExamResponse upsert(UpsertExam request) {
        CourseOffering courseOffering =
                courseOfferingRepository.findById(request.courseOfferingId())
                        .orElseThrow(() ->
    ...
    private void validateCoefficient(UpsertExam request) {
        List<Exam> exams =
                examRepository.findByCourseOffering_Id(
        ...
        if (total.compareTo(BigDecimal.ONE) > 0) {
            throw new BadRequestException(
                    "The sum of exam coefficients cannot exceed 1");
        }
    }
